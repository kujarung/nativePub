      ...
      <View style={[common.contentCon, {marginTop: 15}]}>
        ...
        <View style={{marginBottom: 26}}>
          ...
          <View style={interest.autoCompleteWarp}>
            {autoList.map((val) => (
              <View style={interest.autoCompleteContainer} key={val.id}>
          ...
          {list.map((item) => {
            return (
              <TouchableOpacity
                ...
                style={[
                  interest.selectItem,
                  item.active ? interest.selectedCotainer : null,
                ]}
                key={item.id}>
                <Text
                  style={[
                    interest.selectItemFont,
                    item.active ? interest.selectedFont : null,
                  ]}>
                  {item.title}
                </Text>
              </TouchableOpacity>
